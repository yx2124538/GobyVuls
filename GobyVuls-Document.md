...
|   **Vulnerability**  |Zimbra Collaboration Suite /h javax.servlet.include.path_info File Inclusion Vulnerability (CVE-2025-68645)|
| :----:   | :-----|
|  **Chinese name**  |Zimbra Collaboration Suite /h javax.servlet.include.path_info 文件包含漏洞（CVE-2025-68645）|
| **CVSS core**  | 		6.5 |
| **FOFA Query**  (click to view the results directly)|  [app="zimbra-邮件系统"]
| **Number of assets affected**  | 70W+ |
| **Description**  |A Local File Inclusion (LFI) vulnerability exists in the Webmail Classic UI of Zimbra Collaboration (ZCS) versions 10.0 and 10.1. Due to improper handling of user-supplied request parameters in the RestFilter servlet, an unauthenticated remote attacker can craft malicious requests to the /h/rest endpoint to influence internal request dispatching and include arbitrary files from the WebRoot directory.|
| **Impact** | An unauthenticated attacker can read sensitive configuration files, account information, and application source code on the server, which may facilitate further attacks and pose a significant security risk.|

![](https://img.cdn1.vip/i/69539d412cc20_1767087425.webp)
...
|   **Vulnerability**  | Next.js Code Execution Vulnerability（CVE-2025-55182）|
| :----:   | :-----|
|  **Chinese name**  |Next.js  代码执行漏洞|
| **CVSS core**  | 		10.0  |
| **FOFA Query**  (click to view the results directly)|  [header="Next.js" && body="/_next/static"]
| **Number of assets affected**  | 800W+ |
| **Description**  |Next.js is a popular React framework widely used for building modern web applications.CVE-2025-55182 mainly affects the Server Actions feature of react-server-dom-webpack. Due to the lack of security checks when parsing client RSC-related requests, attackers can craft malicious requests to execute arbitrary code on the server and even take full control of the service. Additionally, because Next.js versions 15.x and 16.x rely on the flawed React Server DOM package when using the App Router, attackers can also inject malicious code to execute remote commands.|
| **Impact** | An attacker can exploit this vulnerability to execute arbitrary code on the server, install a backdoor, gain server privileges, and ultimately take control of the entire web server.|

![](https://img.cdn1.vip/i/69329e1a85913_1764924954.webp)
...
| **Description**  |Oracle Identity Manager is an identity management solution provided by Oracle. This vulnerability exists in the /iam/governance/applicationmanagement/api/v1/applications/groovyscriptstatus;.wadl interface. Attackers can execute arbitrary commands on the target server by sending specially crafted Groovy scripts, potentially leading to sensitive information leakage and full system compromise. |
| **Impact** | This vulnerability allows attackers to execute arbitrary commands on the target server using specially crafted Groovy scripts, potentially leading to full server control, sensitive data leakage, and service interruptions.|
| **Affected versions** |12.2.1.4.0, 14.1.2.1.0|

![](https://img.cdn1.vip/i/69255d354b68e_1764056373.webp)
